...
- 共通前提は `AGENTS.md` / `docs/index.md` / `docs/architecture.md` / `docs/dto.md` / `docs/testing.md` / `docs/logging.md` / `docs/operations.md` / `docs/setup.md` に従う
- UI作業では `docs/ui-development-flow.md` / `docs/frontend.md` / `docs/ui.md` に従う
- 実装作法、型、CI必須ゲートと手元確認コマンドは `docs/coding-standards.md` に従う
- コメント・アノテーション方針は `docs/commenting.md` に従う
- 秘密情報は扱わない

## Git運用

- main 直pushは禁止
- 作業ブランチは作業目的に合わせて `feature/xxx`、`fix/xxx`、`docs/xxx` 形式で作成する
...
- 工程: MOCK / PROTOTYPE / PRODUCT
- MOCKで確認するUI契約:
- PROTOTYPEで確認する導線:
- PRODUCTへ引き継ぐComponent構造:
- PRODUCTで置き換えるprops:
- 本データ接続点:
- 引き継がない固定データ・検証用ロジック:
