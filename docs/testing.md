...
Vitest を確認する場合は npm コンテナで実行します。
...
docs のみの変更では、アプリテストの実行は必須ではありません。ただし、Markdown表示、リンク、差分範囲を確認します。
...
CIの `composer format-check` は `pint --test` を実行する前提です。CIを通すために `pint --test --dirty` へ弱めません。

`npm run typecheck` は削除しませんが、既存型エラーが残っている間は CI 必須ゲートに戻しません。TypeScript / TSX 変更時の手元確認コマンドとして扱います。
